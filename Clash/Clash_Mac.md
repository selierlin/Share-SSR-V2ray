# Clash Party macOS 使用教程

Clash Party 是基于 Mihomo 内核的图形客户端，支持在 macOS 上导入订阅、选择节点和配置系统代理。本教程以常见订阅链接为例说明基础使用流程。

> ClashX 已长期停止维护，不建议新用户继续使用。本教程不再适用于 ClashX。

## 1. 下载与安装

1. 前往 [Clash Party 官方下载页](https://github.com/mihomo-party-org/clash-party/releases/latest)。
2. 根据 Mac 的芯片类型选择对应的 macOS `pkg` 安装包：Apple 芯片（M 系列）请选择 Apple Silicon 版本，Intel 芯片请选择 Intel 版本。
3. 双击下载的 `pkg` 文件，按 macOS 安装向导完成安装。
4. 首次启动 Clash Party 时，如系统请求网络或系统代理相关权限，请按提示授权。
...
3. 打开左侧的「系统代理」开关。首次启用时如 macOS 要求输入管理员密码或授权，请按提示操作。
